/* 
    kontributor :
    1. Mochammad Shaffa Prawiranegara
    2. Ariq Pradipa Santoso
*/
...
void displayUser(struct listing *head, int userCount);  // mendisplay hasil linked list
...
int userCari(int refill) {
    ...
    if(fileUser == NULL) {  // jika file tiadk ketemu
    ...
    while(fgets(stringUser, sizeof(stringUser), fileUser) != NULL) {
        ...
        // stripped mengambil stingUser hingga line baru
        stripped = strtok(stringUser, "\n");
...
int userDeposit(int userSelect) {
    ...
    remove("data/fileUser.txt");                      // menghapus file
    rename("data/tempUser.txt", "data/fileUser.txt"); // merename file
